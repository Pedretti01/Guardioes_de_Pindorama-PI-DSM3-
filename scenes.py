...
class Level(Scene):
    ...
    def __init__(self): 
        ...
        self.img = Obj("assets/levelSprite/level.png", [0, 0], [self.all_sprites])  # Fundo da fase
        ...
        self.dialogue = Dialogo.falas[:5]  # Primeiro conjunto de diálogos
        self.questions = Questoes.perguntas  # Todas as questões
        self.final_dialogue = Dialogo.falas[5:]  # Últimos diálogos
        ...
        self.questions = Questoes.perguntas[:]
        ...
        if event.type == pygame.KEYDOWN:  # Certifique-se de que é um evento de tecla
            ...
            if event.key == pygame.K_e and self.player.rect.colliderect(self.npc.rect) and not self.chatbox.is_active():
                if self.dialogue_stage == 0:  # Diálogo inicial
                    ...
                    formatted_dialogue = [f"{speaker} {message}" for speaker, message in Dialogo.falas[:5]]
    ...
    def update(self):
        ...
        self.hud.update_live(self.player.lives)  # Atualiza o número de vidas
